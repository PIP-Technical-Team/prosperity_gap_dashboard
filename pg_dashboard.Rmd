...
dt_pipr_fg <- fst::read.fst( 
               path = fs::path("dt_pipr_fg.fst"))
pipversion <- qs::qread(file = fs::path("pipversion.qs"))

url_data     <- "https://github.com/PIP-Technical-Team/prosperity_gap_data/"
url_raw_file <- "https://raw.githubusercontent.com/PIP-Technical-Team/prosperity_gap_data/dev/"
#my_token     <- Sys.getenv("GITHUB_PAT")

read_data <- function(file, 
                      url = url_raw_file, #usr_name, 
                     # token, 
                      fst = TRUE, 
                      csv = FALSE) {
  ...
  writeBin(httr::content(response, 
                         as = "raw"), 
           temp_file)
  if (fst) {
    data <- fst::read_fst(temp_file,
                          as.data.table = TRUE)
  } else if (csv) {
...
fst_files <- c(paste0("dt_", 
                    c("lineup", "region", "country", "imputed"),
                    ".fst"), 
               "headcounts_lineup.fst",
               "headcounts_survey.fst",
               "countries_lookup.fst"
               )

# Apply the function to each CSV file
data_list <- lapply(fst_files, function(file_name) {
  read_data(url = url_raw_file, file_name)#, token = my_token)
})

names(data_list) <- c("lineup", "region", "country", 
                      "imputed", "hc_lineups", "hc_survey", "countries_lookup")

# DATA OBJECTS ####
dt_country          <- data_list$country |> 
  fmutate(Survey_grouping      = paste0(Country_code, Reporting_level, Welfare_type))
dt_region           <- data_list$region
dt_lineup           <- data_list$lineup |> 
  fmutate(Survey_comparability = paste0(Country_code, Reporting_level, Welfare_type)) |> 
  fmutate(Survey_grouping      = Survey_comparability)
countries_lookup    <- data_list$countries_lookup
dt_headcount_survey <- data_list$hc_survey  #|> 
  #fmutate(Survey_grouping      = Survey_comparability)
dt_headcount_lineup <- data_list$hc_lineups #|> 
  #fmutate(Survey_grouping      = Survey_comparability)
dt_imputed          <- data_list$imputed #|> 
  #fmutate(Survey_grouping      = Survey_comparability)


dt_headcount_survey <- left_join(x = dt_headcount_survey, 
                                 y = dt_country |> 
                                   fselect(Country_code, 
                                           Year, 
                                           Welfare_type, 
                                           Reporting_level), 
                                 by = c("Country_code", "Year", "Reporting_level", "Welfare_type"), 
                                 reportvar = FALSE) #|> 
  #fmutate(Survey_comparability = paste0(Country_code, Reporting_level, Welfare_type))
dt_headcount_lineup <- left_join(x = dt_headcount_lineup,
                                 y = dt_pipr_fg |> 
                                   frename(country_code    = Country_code,
                                           year            = Year,
                                           welfare_type    = Welfare_type,
                                           reporting_level = Reporting_level) |> 
                                   fselect(Country_code, Year, Welfare_type, Reporting_level) |> 
                                   fsubset(!(Country_code %in% c("IND", "CHN") & !Reporting_level == "national")),
                                 by = c("Country_code", "Year", "Reporting_level", "Welfare_type"),
                                 reportvar = FALSE) |>
  fmutate(Survey_comparability = paste0(Country_code, Reporting_level, Welfare_type)) |> 
  fmutate(Survey_grouping = Survey_comparability)
...
source_text <- paste0("Kraay ⓡ al. (2023) updated based on Poverty and Inequality Platform (version ", pipversion, ")")
...
title_function <- function(ind){
  ...
  title_use_vec <- c(
    "Mean (daily per capita, 2021 $PPP)", 
    #"Mean of bottom 40 (daily pc, 2021 $PPP)",
    "Prosperity Gap", 
    "(PG) Inequality", 
    "Growth in Prosperity Gap", 
    "Growth in Mean (daily p.c., 2021 $PPP)", 
    #"Growth in Mean of bottom 40 (daily pc, 2021 $PPP)",
    "Growth in (PG) Inequality", 
    "Poverty headcount - $3.00 daily p.c.",
    "Poverty headcount - $4.20 daily p.c.", 
    "Poverty headcount - $8.30 daily p.c."
  )
...
title_function2 <- function(ind){
  ...
  title_lookup <- c(
    ...
    "Poverty headcount - $3.00",
    "Poverty headcount - $4.20",
    "Poverty headcount - $8.30"
  )
  
  title_use_vec <- c(
    "Prosperity Gap",
    "Mean (daily p.c., 2021 $PPP)", 
    #"Mean of bottom 40 (daily p.c.)", 
    "(PG) Inequality", 
    "Growth in Prosperity Gap", 
    "Growth in Mean (daily pc, 2021 $PPP)", 
    #"Growth in Mean of bottom 40 (daily pc, 2021 $PPP)",
    "Growth in (PG) Inequality", 
    "Poverty headcount - $3.00 (daily p.c.)",
    "Poverty headcount - $4.20 (daily p.c.)", 
...
create_data_imputed_by_countries_two_years_pg <- function(
  ...
  dt1 <- dt1[, .(Country_name, Year, Indicator1, ImputeYears1, Impute_Forward1, Survey_comparability, Survey_grouping)]
...
dt_trend_plot1 <- reactive({
  ...
  if (input$plot_which_trend == "Regional aggregates") {
    ...
    dt_trend_plot1[, Survey_grouping := 1L]
  ...
  if (input$plot_which_trend == "Selected economies") {
    ...
    indicator_trend1 <- switch(
      ...
      "Growth - Mean"             = "Mean_growth" , 
      "Growth - (PG) Inequality"  = "Inequality_growth", 
      "Poverty headcount - $3.00" = "Headcount3", 
      "Poverty headcount - $4.20" = "Headcount420", 
      "Poverty headcount - $8.30" = "Headcount830")
    }
    ...
    dt_trend_plot1[
      ...
      text_tooltip := (
        paste0(
          "Economy: ", Economy, "\n",
          "Year: ", Year, "\n", 
          input$indicator_trend1_economies, ": ", 
          round(IndicatorTrend1, 2), "\n",
  ...
  if (input$plot_which_trend == "Both") {
    ...
    dt_trend_plot1[
      ...
      text_tooltip := (
        paste0(
          "Economy: ", Economy, "\n",
          "Year: ", Year, "\n", input$indicator_trend1_both, 
          ": ", round(IndicatorTrend1, roundto)
        )
      )
    ]
...
dt_trend_plot2 <- reactive({
  ...
  if (input$plot_which_trend == "Regional aggregates") {
    ...
    dt_trend_plot2[
      ...
      text_tooltip := (
        paste0(
          "Economy: ", Economy, "\n",
          "Year: ", Year, "\n", 
          input$indicator_trend2_regionalaggregates, ": ", 
          round(IndicatorTrend2, roundto)))
    ]
    ...
    dt_trend_plot2[, Survey_grouping := 1L]
    ...
    if ( grepl(pattern = "headcount", x = input$indicator_trend2_economies) == TRUE ) {
      if (input$lineup_or_survey_trend == "Survey") {
        ...
        dt_trend_plot2 <- copy(dt_headcount_lineup)
          ...
          } else if (input$lineup_or_survey_trend == "Interpolated") {
    ...
    dt_trend_plot2[
      ...
      text_tooltip := (
        paste0(
          "Economy: ", Economy, "\n",
          "Year: ", Year, "\n", 
          input$indicator_trend2_economies, ": ", 
          round(IndicatorTrend2, roundto), "\n",
          "Reporting level: ", Reporting_level, "\n", 
          "Welfare type: ", Welfare_type
        )
...
dt_use <- reactive({
  ...
  # dt_use
  #dt_use <- dt_use[, !("Region_name.y")]  # Remove Region_name.y
#setnames(dt_use, "Region_name.x", "Region_name")  # Rename Region_name.x to Region_name
  # Filter years ----
  dt_use <- dt_use[
...
dt_use_sp <- reactive({
  ...
  dt_use_sp[
    ,
    `Income Group` := incgroup
  ]
  #factor(incgroup, levels = incgroup[order(mean)])]
  # Define tooltip
  dt_use_sp[
    ,
    text_tooltip := (
...
<!-- {data-width=500} -->
<p style='text-align:justify;'>This dashboard provides a set of interactive visualizations focused on the *Prosperity Gap* and associated indicators. The Prosperity Gap is the average factor by which incomes need to be multiplied to bring everyone in the society to the prosperity standard of \$28 daily income expressed in 2021 PPPs. A Prosperity Gap of 2.8 implies that incomes in that society need to be increased on average 280\% for everyone to reach the \$28/day threshold. The global prosperity standard is set at $28 per person per day—roughly equal to the average per capita household income at the point where countries transition from upper-middle-income to high-income status, according to the World Bank’s income classification (Kraay ⓡ al. 2023; Foster ⓡ al. 2025).</p>

<p style='text-align:justify;'>Other indicators include the mean of the distribution and an inequality measure corresponding to the Prosperity Gap— *(PG) Inequality* —measure. (PG) Inequality is the average factor by which incomes need to be multiplied to bring everyone in the society to the mean income of that society. The dashboard can be used to compare the measures of a country, region, or the world across time or between each other.</p>
...
<p style='text-align:justify;'>Please cite the data as:  “Kraay ⓡ  al (2023) updated using Poverty and Inequality Platform (version `r pipversion`).” .</p>


<p style='text-align:justify;'>References:</p>
<p style='text-align:justify;'>Foster, Elizabeth Mary; Jolliffe, Dean Mitchell; Lakner, Christoph; Lara Ibarra, Gabriel; Tetteh Baah, Samuel Kofi. (2025)
Global Poverty Revisited Using 2021 PPPs and New Data on Consumption (English). Policy Research Working Paper; Prosperity; RRR Washington, D.C. : World Bank Group. [http://documents.worldbank.org/curated/en/099503206032533226](http://documents.worldbank.org/curated/en/099503206032533226)</p>

<p style='text-align:justify;'>Kraay, Aart C.; Lakner, Christoph; Ozler, Berk; Decerf, Benoit Marie A; Jolliffe, Dean Mitchell; Sterck, Olivier; Yonzan, Nishant. (2023)
A New Distribution Sensitive Index for Measuring Welfare, Poverty, and Inequality (English). Policy Research working paper ; no. WPS 10470; RRR Washington, D.C. : World Bank Group. [http://documents.worldbank.org/curated/en/099934305302318791](http://documents.worldbank.org/curated/en/099934305302318791) </p>
...
\newline
<p style='text-align:justify;'> *Prosperity Gap* - the average factor by which incomes need to be multiplied to bring everyone in the society to the prosperity standard of $28 daily income expressed in 2021 PPPs. </p>

<p style='text-align:justify;'> *Mean* - the daily per person mean income of the whole distribution expressed in 2021 PPPs. </p>

<p style='text-align:justify;'> *Mean of the bottom 40\%* - the daily per person mean income of the bottom 40% of the distribution expressed in 2021 PPPs.</p>
