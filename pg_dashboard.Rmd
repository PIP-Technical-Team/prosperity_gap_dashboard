...
- data/PIPinput_survey_20231016.csv
- data/PIPinput_region_20231017.csv
---
...
global_summary <- dt_region[
  , 
  .(PG = sum(PG), PG_decom = sum(PG_decomp), Inequality = sum(Inequality), Mean = sum(Mean)), 
  by = Year
]
...
# Finally, combine the original data.table and the new "Global" region data.table
dt_region <- rbindlist(
  list(
    dt_region, 
    global_region
...
dt_region <- dt_region[, .(Region_code, Year, PG, PG_decomp, Inequality, Mean, Region_name)]
setorder(
  dt_region, Year
)
...
conditionalPanel(
  condition = "input.plot_which_trend == 'Selected economies' | input.plot_which_trend == 'Both' ", 
  selectInput(
    "lineup_or_survey_trend", 
    label    = "Plot imputed or survey data", 
    choices  = c(
      "Imputed", 
      "Survey"
    ), 
...
dt_trend_plot1 <- reactive({
  ...
  if(input$plot_which_trend == "Both"){
    ...
    dt_trend_c1 <- dt_trend_c1[
      , 
      .(Economy=Country_name, Year, PG, Inequality, Mean, Region_code, Survey_comparability)
    ]
    dt_trend_r1 <- copy(dt_region)
    dt_trend_r1 <- dt_trend_r1[!Region_name == "Global"]
    dt_trend_r1 <- dt_trend_r1[
      , 
      .(Economy=Region_name, Year, PG, Inequality, Mean, Region_code, Survey_comparability = 1L)
    ]
    ...
    dt_trend_plot1[
      ...
      text_tooltip := (
        paste0(
          "Economy: "                     , Economy, "\n",
          "Year: "                        , Year,
          "\n"                            , input$indicator_trend1_both, ": ", round(IndicatorTrend1, 2)
        )
      )
    ]
...
tp1 <- layout(tp1,
            ...
            yaxis = list(
              title = title_function2(ind_name),
...
renderPlotly({
  ...
  # give name
  if(input$plot_which_trend == "Regional aggregates"){
    ind_name <- input$indicator_trend2_regionalaggregates
  } else if(input$plot_which_trend == "Selected economies"){
    ind_name <- input$indicator_trend2_economies
  } else {
    ind_name <- input$indicator_trend2_both
  }
...
tp2 <- tp2 +
  ...
  geom_point(aes(x = round(Year), y = transformation(IndicatorTrend2), color = Economy, text = text_tooltip), size = 1.5, alpha = 0.9) +
  scale_x_continuous(expand = expand_scale(mult = c(0.2, 0.2)))+
  scale_color_brewer(palette = "Dark2")
...
tp2 <- layout(tp2,
            title = list(
              text = paste("Figure 2:", title_function2(ind_name), "over time"),
              titlefont = list(
                size = 14
              )
              ),
...
if(input$level_or_growth_decomposition == "Level"){
  ...
  hc_yAxis(title = list(text = "Prosperity Gap"), #title_function2(input$indicator_barchart)), 
           gridLineWidth =0) |>
  hc_xAxis(categories = unique(dt$Year)) |>
  hc_legend(reverse = TRUE) |> 
  hc_credits(enabled = TRUE, text = paste0("Source: ", source_text, ". Accessed on ", today())) |>
  hc_title(text = paste0("Decomposition of ", "Prosperity Gap" #title_function2("Prosperity G")
                         , " by region")) |> 
  hc_add_theme(my_own_theme) |> 
  hc_xAxis(categories = list("Europe & blabla Asia", "Latin America & Caribbean","Middle East & North Africa", "Other High Income Countries","South Asia", "Sub-Saharan Africa", "East Asia & Pacific" )) |>
  ...
  hc_plotOptions(column = list(stacking = "normal")) |>
  hc_yAxis(title = list(text = "Annual growth rates"), 
           gridLineWidth =0) |>
  hc_xAxis(categories = unique(dt$Year)) |>
  hc_legend(reverse = TRUE) |> 
  hc_credits(enabled = TRUE, text = paste0("Source: ", source_text, ". Accessed on ", today())) |>
  ...
  hc_add_series(data = dt_pg, type = "line", hcaes(x = Year, y = Indicator, group = Group), color = "black")
...
## Plot single region aggregate or 
radioButtons("plot_which_change", 
                   label = "Plot:",  
                   choices = c(
                     "Regional aggregates", 
                     "Countries"
...
# observeEvent(
#   input$interval_change, 
#   {
#     updated_max <- input$interval_change
#     updateSelectInput(
#       session, 
#       "window_change", 
#        min   = 0, 
...
pp <- layout(pp,
            ...
            xaxis = list(
              title = title_function(indicator_change),
              titlefont = list(size = 12)
            ),
            yaxis = list(
              title = ""
...
table <-
  reactive({
    ...
    if("(PG) Inequality" %chin% input$ineq_indicator_table){
      setnames(
        table, 
        old = "Inequality", 
        new = "(PG) Inequality"
...
renderPlotly({
  ...
  nm <- names(table())
  # dt_nm <- data.table(original = nm)
  # dt_nm[, nm := original]
  # dt_nm[original == "Inequality", nm := "(PG) Inequality"]
  # nm <- dt_nm$nm
 # nm <- gsub("\\(PG\\) ", "", nm)

  cn <- table() |> colnames()
  # dt_cn <- data.table(original = cn)
  # dt_cn[, cn := original]
  # dt_cn[original == "Inequality", cn := "(PG) Inequality"]
  # cn <- dt_cn$cn
 # cn <- gsub("\\(PG\\) ", "", cn)
...
<p style='text-align:justify;'> *Prosperity Gap, Inequality* - the average factor by which incomes need to be multiplied to bring everyone in the society to the mean income of that society..</p>
